...
public class TrackerHandler : MonoBehaviour
{
    ...
    public void updateTracker(BackgroundData trackerFrameData, int i_closest){
        if ((int)trackerFrameData.NumOfBodies > i_closest){
            ...
            SortedList<float, int> closestBody = findClosestTrackedBody(trackerFrameData);

            // // render the closest body
            Body skeleton = trackerFrameData.Bodies[closestBody.Values[i_closest]];
            renderSkeleton(skeleton, i_closest);
    ...
    public bool updateBothPeople(BackgroundData trackerFrameData) {
        ...
        } if ((int)trackerFrameData.NumOfBodies >= 2){
            // Get a list of closest bodies
            SortedList<float, int> closestBody = findClosestTrackedBody(trackerFrameData);
            

            // render the closest body
            Body skeleton_1 = trackerFrameData.Bodies[closestBody.Values[0]];
            Body skeleton_2 = trackerFrameData.Bodies[closestBody.Values[1]];

            renderSkeleton(skeleton_1, 0);
            renderSkeleton(skeleton_2, 1);

            if ((skeleton_1.JointPositions3D[0].X > 0.0f && skeleton_2.JointPositions3D[0].X > 0.0f)
                || (skeleton_1.JointPositions3D[0].X < 0.0f && skeleton_2.JointPositions3D[0].X < 0.0f)){
                    return true;
            }
    ...
    public void updateTracker(BackgroundData trackerFrameData)
    {
        ...
        int closestBody = findClosestTrackedBody(trackerFrameData).Values[0];
    ...
    {
        SortedList<float, int> sl = new SortedList<float, int>();
    ...
    {
        // Debug.Log(Y_180_FLIP + " " + Y_180_FLIP.eulerAngles + " " + Quaternion.Euler(0,90,0));
        for (int jointNum = 0; jointNum < (int)JointId.Count; jointNum++)
        {
